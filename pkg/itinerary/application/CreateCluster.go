...
func (c *Cluster) InitializeClusters(lenVisit int, matrixCost *matrixDomain.MatrixCost) {
	...
	for i := 0; i < c.NClusters; i++ {
		...
		c.CentroID[i][0] = (*matrixCost).MinLat*1.001 + rand.Float64()*((*matrixCost).MaxLat*0.999-(*matrixCost).MinLat*1.001)
		c.CentroID[i][1] = (*matrixCost).MinLong*1.001 + rand.Float64()*((*matrixCost).MaxLong*0.999-(*matrixCost).MinLong*1.001)
	}
...
func (c *Cluster) FillCluster(visit *[]placeDomain.Place, change *bool) {
	...
	for i, p := range *visit {
		...
		for j = 0; j < c.NClusters; j++ {
			c.ClusterAlloc[i][j] = p.CalcDistance(c.CentroID[j][0], c.CentroID[j][1])
...
func (c *Cluster) RedefineCentroID() {
	...
	for i := range c.Clusters {
		...
		c.CentroID[i][0] = minLat + rand.Float64()*(maxLat-minLat)
		c.CentroID[i][1] = minLong + rand.Float64()*(maxLong-minLong)
	}
}
